...
class RelievingLetterBottomSheet extends StatefulWidget {
  final String employeeId;
  final String employeeName;

  const RelievingLetterBottomSheet({super.key, required this.employeeId, required this.employeeName});

  @override
  State<RelievingLetterBottomSheet> createState() => _RelievingLetterState();
}

class _RelievingLetterState extends State<RelievingLetterBottomSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await requestRef.set({
        ...
        'description': 'Request for a relieving letter from HR.',
        'status': 'pending',
      ...
      try {
        await NotificationCenter.sendAdmin(
          ...
          message: '${widget.employeeName} requested a relieving letter.',
        );
      } catch (_) {}
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Relieving letter request sent to HR.')),
      );
    } catch (e) {
  if (!mounted) return;
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Text('Could not submit Relieving letter request: $e'),
    ),
  );
}
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.badge_outlined, color: Color(0xFF4C7EDB), size: 21),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Relieving letter', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
                        const SizedBox(height: 2),
                        const Text('Request an official letter confirming your release from employment.', style: TextStyle(fontSize: 10.5, color: AppColors.textSecondary)),
                      ],
                    ),
                  ),
                  Material(
              ...
              Padding(padding: const EdgeInsets.only(bottom: 7), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(margin: const EdgeInsets.only(top: 5), width: 5, height: 5, decoration: const BoxDecoration(color: AppColors.green, shape: BoxShape.circle)), const SizedBox(width: 8), Expanded(child: Text('Date of joining and last working date', style: const TextStyle(fontSize: 10.5, height: 1.35, color: AppColors.textSecondary))) ])),
              Padding(padding: const EdgeInsets.only(bottom: 7), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(margin: const EdgeInsets.only(top: 5), width: 5, height: 5, decoration: const BoxDecoration(color: AppColors.green, shape: BoxShape.circle)), const SizedBox(width: 8), Expanded(child: Text('Confirmation of employment and relieving from duties', style: const TextStyle(fontSize: 10.5, height: 1.35, color: AppColors.textSecondary))) ])),
              ...
              Container(
                ...
                child: const Text('The final letter is issued only after HR verifies the employee record and required clearance.', style: TextStyle(fontSize: 10, height: 1.35, color: AppColors.textSecondary)),
              ),
              const SizedBox(height: 14),
              SizedBox(
                width: double.infinity,
