...
class ExperienceLetterBottomSheet extends StatefulWidget {
  final String employeeId;
  final String employeeName;

  const ExperienceLetterBottomSheet({super.key, required this.employeeId, required this.employeeName});

  @override
  State<ExperienceLetterBottomSheet> createState() => _ExperienceLetterState();
}

class _ExperienceLetterState extends State<ExperienceLetterBottomSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await requestRef.set({
        ...
        'description': 'Request for a experience letter from HR.',
        'status': 'pending',
      ...
      try {
        await NotificationCenter.sendAdmin(
          ...
          message: '${widget.employeeName} requested an experience letter.',
        );
      } catch (_) {}
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Experience letter request sent to HR.')),
      );
   } catch (e) {
  if (!mounted) return;
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Text('Could not submit experience letter request: $e'),
    ),
  );
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.workspace_premium_outlined, color: Color(0xFF8351C7), size: 21),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Experience letter', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
                        const SizedBox(height: 2),
                        const Text('Request an official record of your employment experience.', style: TextStyle(fontSize: 10.5, color: AppColors.textSecondary)),
                      ],
                    ),
                  ),
                  Material(
              ...
              Padding(padding: const EdgeInsets.only(bottom: 7), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(margin: const EdgeInsets.only(top: 5), width: 5, height: 5, decoration: const BoxDecoration(color: AppColors.green, shape: BoxShape.circle)), const SizedBox(width: 8), Expanded(child: Text('Date of joining and employment tenure', style: const TextStyle(fontSize: 10.5, height: 1.35, color: AppColors.textSecondary))) ])),
              Padding(padding: const EdgeInsets.only(bottom: 7), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(margin: const EdgeInsets.only(top: 5), width: 5, height: 5, decoration: const BoxDecoration(color: AppColors.green, shape: BoxShape.circle)), const SizedBox(width: 8), Expanded(child: Text('Role/responsibilities as maintained in the employee record', style: const TextStyle(fontSize: 10.5, height: 1.35, color: AppColors.textSecondary))) ])),
              Padding(padding: const EdgeInsets.only(bottom: 7), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(margin: const EdgeInsets.only(top: 5), width: 5, height: 5, decoration: const BoxDecoration(color: AppColors.green, shape: BoxShape.circle)), const SizedBox(width: 8), Expanded(child: Text('Last working date, where applicable', style: const TextStyle(fontSize: 10.5, height: 1.35, color: AppColors.textSecondary))) ])),
              ...
              Container(
                ...
                child: const Text('The final wording and employment details are verified by HR before the letter is issued.', style: TextStyle(fontSize: 10, height: 1.35, color: AppColors.textSecondary)),
              ),
              const SizedBox(height: 14),
              SizedBox(
                width: double.infinity,
